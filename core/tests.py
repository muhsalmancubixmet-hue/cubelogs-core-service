...
class LeadWorkflowTests(APITestCase):
    ...
    def test_public_lead_ingestion_and_audit(self):
        from company.models import Lead, LeadHistory
...
class BackofficeTests(APITestCase):
    ...
    def test_package_crud_as_superadmin(self):
        from subscribers.models import SubscriptionPackage
        ...
        # Create again with same name (upsert test)
        payload['price'] = '119.99'
        response = self.client.post(url, payload, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(SubscriptionPackage.objects.count(), 1)
        self.assertEqual(SubscriptionPackage.objects.first().price, Decimal('119.99'))
    ...
    def test_coupon_crud_as_superadmin(self):
        from subscribers.models import Coupon
        url = '/api/coupons/'
        self.client.force_authenticate(user=self.superadmin)
...
class UnifiedConnectionFrameworkTests(APITestCase):
    def setUp(self):
        ...
        from company.models import CMSContent, LMSModule
...
class EmployeeBulkUploadTests(APITestCase):
    ...
    def test_bulk_upload_partial_success_and_smtp_failure(self):
        ...
        with patch('django.core.mail.send_mail', side_effect=mock_send_mail), \
             patch('core.tasks.send_mail', side_effect=mock_send_mail):
    ...
    def test_bulk_upload_records_password_in_email_log(self):
        ...
        # Check that welcome email was sent to alice@bulk.com
        from django.core import mail
        email = next((m for m in mail.outbox if 'alice@bulk.com' in m.to), None)
        self.assertIsNotNone(email)
        html_body = email.alternatives[0][0]
        self.assertIn('Welcome to CubeLogs', email.subject)
        self.assertIn('alice@bulk.com', html_body)
        self.assertIn('<strong>Password:</strong>', html_body)
    ...
    def test_employee_raw_password_sync(self):
        ...
        self.assertTrue(emp.check_password('InitialPassword123'))
        
        # Test change password updates password hash
        emp.set_password('NewPassword456')
        emp.save()
        
        # Reload from DB and verify
        emp.refresh_from_db()
        self.assertTrue(emp.check_password('NewPassword456'))
...
class CompanyRegistrationTests(APITestCase):
    ...
    def test_backoffice_register_company_sends_email_and_logs(self):
        ...
        # Verify alternative HTML body contains password
        html_body = email.alternatives[0][0]
